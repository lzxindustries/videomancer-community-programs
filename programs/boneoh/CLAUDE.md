...
**Multi-switch mode selectors** — when S2/S3/S4 (or any run of consecutive switches) are concatenated into a multi-bit mode/operator selector word, the correct concatenation is **lower bit index first (MSB)**:

```vhdl
-- Correct: bit1=S2=MSB, bit2=S3, bit3=S4=LSB → {S2,S3,S4} order
s_operator <= registers_in(6)(1) & registers_in(6)(2) & registers_in(6)(3);

-- Wrong (produces reversed operator — S4 acts as MSB, S2 as LSB):
s_operator <= registers_in(6)(3) & registers_in(6)(2) & registers_in(6)(1);
```

Because the SDK packs switch N into bit N−6 of register 6 (S2→bit 1, S3→bit 2, S4→bit 3), higher switch numbers land in higher bit positions. To preserve the intended {S2,S3,S4} = {MSB,mid,LSB} order, you must concatenate in ascending bit-index order (lowest index first). Concatenating in descending order reverses the word. This is the complement of what feels intuitive and is distinct from the `get_bit_depth` MSB-ordering rule above, which concerns which *switch* should carry the most weight — not which *bit index* to read it from.

The register map comment in the VHD must reflect the actual SDK packing:
```
bit 1: Op S2 MSB   → toggle_switch_8
bit 2: Op S3       → toggle_switch_9
bit 3: Op S4 LSB   → toggle_switch_10
```
...
4/26/2026 Fixed multi-switch mode selector concatenation order bug in all four band filter and bit logic programs (registers_in(6)(3)&(2)&(1) → (1)&(2)&(3)); corrected register map comments to match actual SDK bit packing. Added Multi-switch mode selectors rule to Toggle Switch Patterns section of CLAUDE.md.
